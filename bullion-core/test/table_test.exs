defmodule TableTest do
  ...
  test "set up a game" do
    g = Table.new(%{name: "my great game", buyin_dollars: 20, buyin_chips: 100})
  ...
  test "buyin increments counter" do
    {plid, g} = Table.new(%{name: "my great game", buyin_dollars: 20, buyin_chips: 100})
      |> Table.add_player("Tyler")
    assert(Table.total_buyins(g) == 0)
    {_player, g} = Table.buyin(g, plid)
    assert(Table.total_buyins(g) == 1)
    {player_chips, player_value} = Table.player_balance(g, plid)
